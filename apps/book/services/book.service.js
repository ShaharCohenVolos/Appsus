...
function getById(id) {
  ...
  if (!book) return Promise.reject('404')
  return Promise.resolve(book)
}

function addBook(authors, categories, language, pageCount, publishedDate, thumbnail, title) {
  const book = _createBook(authors, categories, language, pageCount, publishedDate, thumbnail, title)
  let books = _loadFromStorage()
  books = [book, ...books]
  _saveToStorage(books)
  return Promise.resolve(book)
...
function addReview(bookId, review) {
  review = _createReview(review)
  const books = _loadFromStorage()
  const book = books.find(book => book.id === bookId)
  book['reviews'] = book['reviews'] ? [review, ...book['reviews']] : [review]
  _saveToStorage(books)
...
function getPrevBookId(bookId) {
  ...
  const nextBookIdx = (bookIdx - 1 === -1) ? books.length - 1 : bookIdx - 1
...
function _createBook(authors, categories, language, pageCount, publishedDate, thumbnail, title) {
  return {
    authors,
    categories,
    description: '',
